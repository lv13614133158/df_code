...
#include "pthread.h"
#include "TSrvMpu_SDK.h" //linux_ivi接口信息
#include "key_manager.h" // 密钥管理器头文件
...
int tbox_get_info(tboxInfo_t *p_tbox_mcu_info) {
	u8 id[128];
    u32 id_len = 128;
    if (Mpu_GetDeviceId(id, &id_len))
    {
       snprintf(p_tbox_mcu_info->ID,id_len + 1, "%s", id);
    }
    else
    {
        printf("id获取失败\n");
    ...
    u8 vin[128];
    u32 vin_len = 128;
    if (Mpu_GetVehicleVin(vin, &vin_len))
    {
        snprintf(p_tbox_mcu_info->VIN,vin_len + 1, "%s", vin);
    }
    else
    {
        printf("vin获取失败\n");
	...
	u32 hwVer = Mpu_GetHWVer();
	snprintf(p_tbox_mcu_info->MANUFACTURER, sizeof(p_tbox_mcu_info->MANUFACTURER) + 1, "0x%08X", hwVer);
    u32 ver = 0;
    u32 date = 0;

    if (Mpu_GetVersionInfo(&ver, &date))
    {
        u8 year = (date >> 16) + 2000;
        u8 month = (date >> 8) & 0xFF;
        u8 day = date & 0xFF;
		snprintf(p_tbox_mcu_info->SYS_VERSION, sizeof(p_tbox_mcu_info->SYS_VERSION) + 1, "0x%08X", ver);
        printf("软件版本号: 0x%08X\n", ver);
        printf("软件日期: %d年%d月%d日\n", year, month, day);
    }
    else
    {
        printf("软件版本获取失败\n");
...
// 	tbox_mcu_info_t tbox_mcu_info;
	...
	//int initres = sec_api_init();
	//printf("api init res::%d\n", initres);
	while (1)
	{
		...
		if (isString(tbox_mcu_info.VIN) && isString(tbox_mcu_info.ID))
		{
			...
			{
				printf("vin = %s, id = %s\n",tbox_mcu_info.VIN, tbox_mcu_info.ID);
...
exit:	
	...
	snprintf(tboxInfo_obj.VIN, sizeof(tboxInfo_obj.VIN), "%s", tbox_mcu_info.VIN);
	snprintf(tboxInfo_obj.ID, sizeof(tboxInfo_obj.ID), "%s", tbox_mcu_info.ID);
	//snprintf(tboxInfo_obj.MANUFACTURER, sizeof(tboxInfo_obj.MANUFACTURER), "%s", tbox_mcu_info.MANUFACTURER);
	snprintf(tboxInfo_obj.CAR, sizeof(tboxInfo_obj.CAR), "%s", tbox_info_local.CAR);
	memset(spdlog, 0 ,sizeof(spdlog));
...
int initCert(void)
{
	uint8_t *cert=NULL;
	int  result;
	uint32_t read_len = 1024*8;
	printf("init cert\n");
	/*create client certificate*/

	cert=malloc(read_len);
	if (!cert) {
        fprintf(stderr, "Error: Memory allocation failed\n");
        return YWKM_ERROR_MEMORY_ALLOC;
    }
	result = ywkm_load_key("dfcv-ids-ca_crts_pem", cert, &read_len);
    if (result != YWKM_SUCCESS) {
        fprintf(stderr, "Key load failed: %s (error code: %d)\n", 
                ywkm_get_error_string(result), result);
        free(cert);
        return result;
    }else
    {
		s_root_cert = (char *)malloc(read_len + 1);
		if (s_root_cert)
		{
			memset(s_root_cert, 0, read_len + 1);
			memcpy(s_root_cert, cert, read_len);
		}else{
			printf("s_root_cert malloc error");
			return -1;
		}
		if(cert)
		{
	...
	cert=(char*)malloc(read_len);
	if (!cert) {
        fprintf(stderr, "Error: Memory allocation failed\n");
        return YWKM_ERROR_MEMORY_ALLOC;
    }
	result = ywkm_load_key("dfcv-ids-client_crts_key_pem", cert, &read_len);
    if (result != YWKM_SUCCESS) {
        fprintf(stderr, "Key load failed: %s (error code: %d)\n", 
                ywkm_get_error_string(result), result);
        free(cert);
    ...
    {
		s_dev_cert = (char *)malloc(read_len + 1);
		if (s_dev_cert)
		{
			memset(s_dev_cert, 0, read_len + 1);
			memcpy(s_dev_cert, cert, read_len);
		}else{
			printf("s_dev_cert malloc error");
			return -1;
		}
		if(cert)
		{
	...
	result = ywkm_load_key("dfcv-ids-client_key_pem", cert, &read_len);
    if (result != YWKM_SUCCESS) {
        fprintf(stderr, "Key load failed: %s (error code: %d)\n", 
                ywkm_get_error_string(result), result);
        free(cert);
    ...
    {
		s_dev_key = (char *)malloc(read_len + 1);
		if (s_dev_key)
		{
			memset(s_dev_key, 0, read_len + 1);
			memcpy(s_dev_key, cert, read_len);
		}else{
			printf("s_dev_key malloc error");
			return -1;
		}
		if(cert)
		{
