...
static char* extract_certificate_by_subject(const char* pem_data, const char* target_subject, int* out_len) {
    if (!pem_data || !target_subject) return NULL;
    
    const char* current_pos = pem_data;
    
    while (current_pos) {
        // 查找证书开始位置
        const char* cert_start = strstr(current_pos, "-----BEGIN CERTIFICATE-----");
        if (!cert_start) break;
        // 查找证书结束位置
        const char* cert_end = strstr(cert_start, "-----END CERTIFICATE-----");
        if (!cert_end) break;
        cert_end += strlen("-----END CERTIFICATE-----");
        // 在证书块前面查找subject信息（通常在证书之前）
        const char* search_start = (cert_start > pem_data + 1024) ? cert_start - 1024 : pem_data;
        const char* subject_line = NULL;
        // 向前搜索最近的subject行
        for (const char* p = cert_start - 1; p >= search_start; p--) {
            if (*p == '\n' && strncmp(p + 1, "subject=", 8) == 0) {
                subject_line = p + 1;
                break;
            }
        }
        // 如果找到了subject行，并且包含目标subject
        if (subject_line) {
            char subject_pattern[256];
            snprintf(subject_pattern, sizeof(subject_pattern), "/CN=%s", target_subject);
            
            if (strstr(subject_line, subject_pattern)) {
                // 找到了匹配的证书，提取它
                int cert_len = cert_end - cert_start;
                char* result = malloc(cert_len + 1);
                if (result) {
                    memcpy(result, cert_start, cert_len);
                    result[cert_len] = '\0';
                    if (out_len) *out_len = cert_len;
                    return result;
                }
            }
        }
...
int wbsClient_init(void)
{
	...
	if (get_pki_client_cert())
	{
		...
		client_cert_buff = malloc(client_cert_len+1);
		if (client_cert_buff)
		{	
			
			client_cert_buff = extract_certificate_by_subject(get_pki_client_cert(), "IVI", &client_cert_len);
			client_cert_buff[client_cert_len] = '\0';
			//printf("client_cert_buff:\n%s\n", client_cert_buff);
		}
	}
	...
	if (get_pki_client_private_key())
	{
		...
		client_private_key_buff = malloc(client_private_key_len+1);
		if (client_private_key_buff)
		{
			memcpy(client_private_key_buff, get_pki_client_private_key(), client_private_key_len);
			client_private_key_buff[client_private_key_len] = '\0';
			//printf("client_private_key_buff:\n%s\n", client_private_key_buff);
		}
	}
