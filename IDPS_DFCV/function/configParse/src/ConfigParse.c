...
#elif POS_3
#define ROOT_PATH_RW  "/usrdata/firmware/idps"
#define ROOT_PATH_OR  "/usrdata/appset/conf/idps"
#elif POS_4
...
#define DEVICE_INFO_PATH   ROOT_PATH_OR "/device_info.conf"
#define BASE_CONFIG_PATH   ROOT_PATH_OR "/base_config.json"
#define POLICY_CONFIG_PATH ROOT_PATH_RW "/conf/policy_config.json"
...
static void * get_gps_task(void *arg)
{
	...
	unsigned int unintlongitude,unintlatitude;
	float flatitude, flongitude;
	while(1)
	{
		if(ids_GetGnssPointInfo(&unintlatitude, &unintlongitude) == 1)
		{
			flongitude=unintlongitude/3600000.0;
			flatitude=unintlatitude/3600000.0;
			
		}else{
			flongitude=0;
			flatitude=0;
		}
		wbsClient_setPosition(flatitude, flongitude);
		sleep(30);
	}
...
// 	tbox_mcu_info_t tbox_mcu_info;
	...
	while (1)
	{
		...
		if(0 == ids_getTboxParam(&tbox_mcu_info))
		{
			memset(spdlog, 0 ,sizeof(spdlog));
			snprintf(spdlog, sizeof(spdlog),
					"vin:%s, sn:%s, supplierInfo:%s, hardwareVersion:%s, softwareVersion:%s\n",
					tbox_mcu_info.vin, tbox_mcu_info.serial_number, tbox_mcu_info.supplier_code, tbox_mcu_info.hardware_version, tbox_mcu_info.soft_version);
			log_d("ConfigParse", spdlog);

			/*判断是否为字符串*/
			if (isString(tbox_mcu_info.vin) && isString(tbox_mcu_info.serial_number))
			{
				/*判断字符串长度*/
				if (strlen(tbox_mcu_info.vin) == 0 || strlen(tbox_mcu_info.serial_number) == 0)
				{
				...
				{
					/*判断字符串是否全部由0组成*/
					if (isAllZero(tbox_mcu_info.vin) || isAllZero(tbox_mcu_info.serial_number))
					{
...
exit:	
	...
	strncpy(tboxInfo_obj.ID, tbox_mcu_info.serial_number, sizeof(tboxInfo_obj.ID) - 1);
	strncpy(tboxInfo_obj.MANUFACTURER, tbox_mcu_info.supplier_code, sizeof(tboxInfo_obj.MANUFACTURER) - 1);
