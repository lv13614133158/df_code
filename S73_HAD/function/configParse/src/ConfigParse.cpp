...
// 保证Cjson数据安全封装
cJSON* getJsonObjectItem_s(cJSON* cJSONBuf,const char *name)
{
	char spdlog[128] ={0};
	if(!cJSONBuf && !name)
	{
...
// 安全获取Cjson数值
double getJsonItemValue_s(cJSON* cJSONBuf, const char *name)
{
	char spdlog[128] ={0};
...
// 并无开辟空间,安全获取Cjson字符串
char* getJsonItemString_t(cJSON* cJSONBuf,const char* name)
{
	char spdlog[128] ={0};
...
// 安全获取Cjson字符串并复制
void getJsonItemString_s(cJSON* cJSONBuf,const char* name, char* valuestr)
{
	char spdlog[128] ={0};
...
void initTboxInfo()
{
	...
	cJSON* SN=NULL;
	cJSON* VIN=NULL;
	cJSON* vehicle_model=NULL;
	cJSON* simu_info=NULL;
	cJSON* sys_version=NULL;
	// 特定接口 getOutInfoAPI，返回值是use_default_info
	

	int ret = -1;
	int fd  = -1;
	std::string   VIN_str,SN_str;
	while (1)
	{
		...
		VIN_str=client.GetVinDataIdentifier();
		SN_str=client.GetEcuSerialNumber();
		strncpy(tbox_s73_had_info.VIN, VIN_str.c_str(), sizeof(tbox_s73_had_info.VIN) - 1);
		strncpy(tbox_s73_had_info.ID, SN_str.c_str(), sizeof(tbox_s73_had_info.ID) - 1);
	...
	device_info = cJSON_GetObjectItem(root,"device_info");
	if(device_info)
	{
		SN = cJSON_GetObjectItem(device_info,"SN");
		...
		VIN = cJSON_GetObjectItem(device_info,"VIN");
		...
		vehicle_model = cJSON_GetObjectItem(device_info,"vehicle_model");
	...
	simu_info = cJSON_GetObjectItem(root,"simu_info");
	if(simu_info)
	{
		sys_version = cJSON_GetObjectItem(simu_info,"sys_version");
