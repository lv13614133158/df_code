 ...
 *                   All rights reserved.
 ***/
#ifndef BOSCH_VIO_INTERFACE_H_
#define BOSCH_VIO_INTERFACE_H_
#ifdef __cplusplus
extern "C" {
#endif
...
#include "bosch_camera_interface.h"
#include "bosch_def.h"
#include "MAX96717F_CustmData.h"
#include "MAX96724_CustomData.h"
 ...
 *
 * @param[in] port      The port number (of type uint32_t) defined in the configuration file.
 ...
 * @design
 */
int32_t hb_cam_eeprom_block_read(uint32_t const port, uint32_t const address, uint32_t const length, uint8_t *const buffer);
 ...
 * @design
 */
int32_t hb_des_get_lock_status(uint32_t const deserial_index, DeserializerLockStatus *const lock_status);

/**
 * @brief read power switch chennel voltage and current.
 *
 * This function read power switch chennel voltage and current by `camera port
 * and stores the result in the `ChannelVoltageCurrentValue` structure.
 *
 * @param[in] port The port number (of type uint32_t) defined in the configuration file.
 * @param[out] data Pointer to a structure that stores the channel voltage and current result.
 *
 * @return Returns 0 on success; otherwise, returns an error code.
 */
int32_t hb_ps_read_voltage_current_value(uint32_t const port, ChannelVoltageCurrentValue *const data);

/**
 * @brief read power switch chennel state.
 *
 * This function read power switch chennel voltage and current by `camera port
 * and stores the result in the `ChannelVoltageCurrentValue` structure.
 *
 * @param[in] deserial_index   The deserializer index
 * @param[out] poc_state Pointer to a structure that stores the channel state.
 *
 * @return Returns 0 on success; otherwise, returns an error code.
 */
int32_t hb_ps_read_channel_state(uint32_t const deserial_index, PSChannelState *const poc_state);
 ...
 *
 * @param[in] desIdx The deserializer index. The index has value 0,1,4.
 ...
 * @return Returns 0 on success; otherwise, returns an error code.
 */
int32_t hb_des_get_errb_status(const uint8_t desIdx, DeserializerErrbStatus * const DesErrbStatus);
 ...
 * @return Returns 0 on success; otherwise, returns an error code.
 */
int32_t hb_cam_get_errb_status(uint32_t const port, CameraModuleErrbStatus * const CamErrbStatus);


/**
 * @brief read the serializer errors.
 *
 * This function read the serializer errors by `port
 * and stores the result in the `SerErrors` structure.
 *
 * @param[in] port   The port number (of type uint32_t) defined in the configuration file.
 * @param[out] SerErrors Stores the errb state.The fileds include blow:
 *
 * @return Returns 0 on success; otherwise, returns an error code.
 */
int32_t hb_cam_get_ser_errors(uint32_t const port, uint64_t * const SerErrors);


/**
 * @brief read the sensor errors.
 *
 * This function read the sensor errors by `port
 * and stores the result in the `SensorErrors` structure.
 *
 * @param[in] port   The port number (of type uint32_t) defined in the configuration file.
 * @param[out] SensorErrors Stores the errb state.The fileds include blow:
 *
 * @return Returns 0 on success; otherwise, returns an error code.
 */
int32_t hb_cam_get_sensor_errors(uint32_t const port, CameraSensorErrors * const SensorErrors);
 ...
 */
int32_t hb_cam_set_register_value(uint32_t const deviceIdx,
 ...
 * @retval 0 on success; otherwise, returns an error code.
 */
int32_t hb_cam_get_register_value(uint32_t const deviceIdx,
                                  DeviceType const deviceType,
                                  uint16_t const registerAddr,
                                  uint16_t const dataLength,
                                  uint8_t *const dataBuffer);
 ...
 * @return Returns 0 on success; otherwise, returns an error code.
 */
int32_t hb_cam_get_module_error_info(uint32_t const index,
                                     void *const serializerErrorInfo,
                                     void *const sensorErrorInfo,
                                     ModuleErrorReadFlag const errorsToRead);
 ...
 */
int32_t hb_des_get_error_info(uint32_t const index,
                              void *const deserializerErrorInfo,
                              uint8_t *isRemoteError,
                              uint8_t *linkErrorMask);
#ifdef __cplusplus
}
#endif

#endif // BOSCH_VIO_INTERFACE_H_